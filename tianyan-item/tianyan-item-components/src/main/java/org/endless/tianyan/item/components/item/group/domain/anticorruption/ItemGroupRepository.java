package org.endless.tianyan.item.components.item.group.domain.anticorruption;

import org.endless.tianyan.item.common.model.domain.anticorruption.TianyanItemRepository;
import org.endless.tianyan.item.components.item.group.domain.entity.ItemGroupAggregate;

/**
 * ItemGroupRepository
 * <p>资源项分组聚合仓储接口
 * <p>
 * itemCreate 2025/07/21 16:04
 * <p>
 * update 2025/07/21 16:04
 *
 * @author Deng Haozhi
 * @see TianyanItemRepository
 * @since 0.0.1
 */
public interface ItemGroupRepository extends TianyanItemRepository<ItemGroupAggregate> {

}
