...
@Lazy
@Component
public class GameEveDataLoadDrivenAdapterImpl implements GameEveDataLoadDrivenAdapter {

    private static final Pattern SCANNER_PATTERN = Pattern.compile("^\\d{1,10}:$", Pattern.MULTILINE);

    private final GameEveDataLoadTaskFactory gameEveDataLoadTaskFactory;

    public GameEveDataLoadDrivenAdapterImpl(GameEveDataLoadTaskFactory gameEveDataLoadTaskFactory) {
    ...
    public void load(GameEveDataTypeEnum dataType) {
        ...
        try (Scanner scanner = new Scanner(new FileSystemResource(task.filePath()).getInputStream())) {
            ...
            throw new DrivenAdapterFailedException("加载数据失败", e);
        }
    }
}
