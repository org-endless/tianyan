package org.endless.tianyan.sales.components.market.price.price.domain.anticorruption;

import org.endless.tianyan.sales.common.model.domain.anticorruption.TianyanSalesRepository;
import org.endless.tianyan.sales.components.market.price.price.domain.entity.MarketPriceAggregate;

/**
 * MarketPriceRepository
 * <p>市场价格聚合仓储接口
 * <p>
 * create 2025/07/26 18:56
 * <p>
 * update 2025/07/26 18:56
 *
 * @author Deng Haozhi
 * @see TianyanSalesRepository
 * @since 0.0.1
 */
public interface MarketPriceRepository extends TianyanSalesRepository<MarketPriceAggregate> {

}
