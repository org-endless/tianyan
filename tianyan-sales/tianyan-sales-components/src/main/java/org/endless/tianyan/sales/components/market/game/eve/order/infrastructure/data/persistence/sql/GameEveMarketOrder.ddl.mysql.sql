...
CREATE TABLE market_order_game_eve (
    game_eve_market_order_id VARCHAR(255) NOT NULL COMMENT '游戏EVE市场订单ID',
    market_order_id          VARCHAR(255) NOT NULL COMMENT '市场订单ID',
    code                     VARCHAR(255) NOT NULL COMMENT '游戏EVE市场订单编号',
    order_range              VARCHAR(255) NOT NULL COMMENT '游戏EVE市场订单范围',
    ...
    CONSTRAINT PRIMARY KEY pk_market_order_game_eve (game_eve_market_order_id),
    CONSTRAINT UNIQUE KEY uk_market_oge_order_id (market_order_id, remove_at)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COMMENT '游戏EVE市场订单表';
CREATE INDEX idx_market_oge_code ON market_order_game_eve (code);
